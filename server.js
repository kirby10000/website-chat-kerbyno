...
io.on("connection", socket => {
  socket.on("register", name => { users.set(socket.id,{name}); sendUsers(); });
  socket.on("get users", sendUsers);
  function sendUsers() {
    const unique = [...new Set([...users.values()].map(u=>u.name))];
    io.emit("all users", unique.map(name=>({name})));
  }
  ...
  socket.on("chat message", data => {
    ...
    if(rooms.has(data.tab)) io.to(data.tab).emit("chat message", msg);
    else if(data.tab!==sender.name){
      for(const [id,u] of users.entries()) if(u.name===data.tab) io.to(id).emit("chat message", msg);
  ...
  socket.on('pong request',(room,partner)=>{
    socket.join(room);
    for(const [id,u] of users.entries()) if(u.name===partner && id!==socket.id) io.to(id).emit('pong request', room, users.get(socket.id).name);
  });

  socket.on('pong accept',(room,partner)=>{
    socket.join(room);
    for(const [id,u] of users.entries()) if(u.name===partner && id!==socket.id) io.to(id).emit('pong accept', room, users.get(socket.id).name);
...
setInterval(()=>{
  for(const room in pongGames){
    ...
    if(g.ball.x-g.ball.radius<20 && g.ball.y>g.left.y && g.ball.y<g.left.y+g.paddleH){ g.ball.vx*=-1; g.ball.x=20+g.ball.radius; }
    if(g.ball.x+g.ball.radius>380 && g.ball.y>g.right.y && g.ball.y<g.right.y+g.paddleH){ g.ball.vx*=-1; g.ball.x=380-g.ball.radius; }
